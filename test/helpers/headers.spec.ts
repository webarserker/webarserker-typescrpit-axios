import { parseHeaders, processHeaders, flottenHeaders } from '../../src/helpers/header';
...
describe('helpers:headers', () => {
  ...
  describe('flottenHeaders', () => {
    test('should flatten the headers and include common headers', () => {
      ...
      expect(flottenHeaders(headers, 'get')).toEqual({
        Accept: 'application/json',
    ...
    test('should flatten the headers without common headers', () => {
      ...
      expect(flottenHeaders(headers, 'patch')).toEqual({
        Accept: 'application/json'
      });
    });

    test('should do nothing if headers is undefined or null', () => {
      expect(flottenHeaders(undefined, 'get')).toBeUndefined();
      expect(flottenHeaders(null, 'post')).toBeNull();
    });
  });
});
